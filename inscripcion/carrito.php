...
class Inscripcion
{
	private $inscripcion = array();
	
	public function __construct()
	{		
		if(!isset($_SESSION["inscripcion"]))
		{
			$_SESSION["inscripcion"] = null;
			
			$this->inscripcion["id"] = time();
		}
		$this->inscripcion = $_SESSION['inscripcion'];
	}
	...
	{
		$_SESSION["inscripcion"][$propertyName] = $propertyValue;
	...
	{
		return $this -> inscripcion[$propertyName];
	}
...
class Carrito
{
	...
	{
		//primero comprobamos el articulo a añadir, si está vacío o no es un array lanzamos una excepción y cortamos la ejecución
		if(!is_array($articulo) || empty($articulo)) { throw new Exception("Error, el articulo no es un array!", 1);	}

		//nuestro carro necesita siempre un id producto, cantidad y precio articulo
		if(!$articulo["id"] || !$articulo["cantidad"] ) { throw new Exception("Error, el articulo debe tener un id, cantidad y precio!", 1);	}

		//nuestro carro necesita siempre un id producto, cantidad y precio articulo
		if(!is_numeric($articulo["id"]) || !is_numeric($articulo["cantidad"]) || !is_numeric($articulo["precio"])) { throw new Exception("Error, el id, cantidad y precio deben ser números!", 1);	}
		...
		if(!empty($this->carrito))
		{
			...
			{
				//comprobamos si este producto ya estaba en el carrito para actualizar el producto o insertar un nuevo producto	
	...
	public function precio_total()
	{
		...
		if(!is_numeric($this->carrito["precio_total"])) { throw new Exception("El precio total del carrito debe ser un número", 1);	}
	...
	public function articulos_total()
	{
		...
		if(!isset($this->carrito["articulos_total"]) || $this->carrito === null) { return 0; }
		
		//si no es númerico lanzamos una excepción porque no es correcto
		if(!is_numeric($this->carrito["articulos_total"])) { throw new Exception("El número de artículos del carrito debe ser un número", 1);	}
	...
	public function remove_producto($unique_id)
	{
		...
		if($this->carrito === null) { throw new Exception("El carrito no existe!", 1); }

		//si no existe la id única del producto en el carrito
		if(!isset($this->carrito[$unique_id])) { throw new Exception("La unique_id $unique_id no existe!", 1); }
